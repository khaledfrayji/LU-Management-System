...
$retrieveStudentQuery = "SELECT * FROM student WHERE sid = '$sid'";
...
if ($result && $result->num_rows > 0) {
    ...
    $_SESSION['sid'] = $fetch_info['sid'];
    $_SESSION['sname'] = $fetch_info['sname'];
...
if (isset($_POST["update_picture"])) {
    ...
    if (!empty($uploadedFileName) && move_uploaded_file($_FILES["profilePic"]["tmp_name"], $targetFilePath)) {
        $updateQuery = "UPDATE student SET profile_picture = '$targetFilePath' WHERE sid = '$sid'";
...
if (isset($_POST['change_password'])) {
    ...
    $checkCurrentPasswordQuery = "SELECT password FROM student WHERE sid = '$sid'";
    ...
    if ($result) {
        ...
        if (password_verify($currentPassword, $storedPasswordHash) || $currentPassword==$storedPasswordHash) {
            ...
            if ($newPassword !== $retypePassword) {
                ...
                $updateQuery = "UPDATE student SET password = '$hashedNewPassword' WHERE sid = '$sid'";
...
if (isset($_POST['update_email'])) {
    ...
    $checkEmailQuery = "SELECT sid FROM student WHERE email = '$newEmail'";
    ...
    if (empty($errors)) {
        $updateEmailQuery = "UPDATE student SET email = '$newEmail' WHERE sid = '$sid'";
...
<body>
    ...
    <main>
        <div class="info">
        <h1>Hi <span><?php echo isset($fetch_info['sname']) ? $fetch_info['sname'] : ''; ?></span></h1>
            <h1>Your Id is: <span><?php echo isset($fetch_info['sid']) ? $fetch_info['sid'] : ''; ?></span></h1>
